...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  return (
    <>
      ...
      <motion.aside
        className={`fixed top-0 left-0 bottom-0 z-50 w-64 bg-dark-900 border-r border-white/5 flex flex-col transition-transform duration-300 lg:translate-x-0 ${
        ...
        <div className="flex-1 py-6 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          {navItems.map((item, idx) => (
            ...
            >
              ...
              {item.active && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_#6B46FF]" />
              )}
            </button>
          ))}
          ...
          {["Tic-Tac-Toe", "Speed Chess", "Connect 4"].map((game, i) => (
            <button
              key={i}
              className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-gray-400 hover:bg-white/5 hover:text-white transition-colors group"
            >
              <span className="w-1 h-1 rounded-full bg-gray-600 group-hover:bg-white transition-colors" />
              <span className="text-sm">{game}</span>
              {i === 0 && (
                <span className="ml-auto text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded border border-green-500/20">
                  LIVE
                </span>
              )}
            </button>
